...
void I2C_pressure_sensor::SetupXferRecAndExecute(uint8_t devAddr,
...
uint8_t I2C_pressure_sensor::readStatus() {
	uint8_t status;
	uint8_t read = 0xF1;
...
void I2C_pressure_sensor::changeStatus(uint8_t const &status) {
	ITM_output p;
...
bool I2C_pressure_sensor::getModeFromStatus(uint8_t status) {
	return (status >> 7) & 1; // can be done without & 1 because 7 is the last bit
}

bool I2C_pressure_sensor::getDataReadyFromStatus(uint8_t status) {
	return (status >> 6) & 1;
}

void I2C_pressure_sensor::readNewTemperature(uint8_t &temperature) {
...
void I2C_pressure_sensor::ReadTemperatureI2CM(void) {
	//uint8_t status;
	uint8_t temperature;
	ITM_output p;
	...
	if (getDataReadyFromStatus(readStatus())) { /*if status (6th bit) is 1 */
		...
		temperatureText = "Temperature read over I2C is "
				+ std::to_string(temperature) + " degrees C.\r\n";
	} else {
		temperatureText = "Error " + std::to_string(i2cmXferRec.status)
				+ " reading temperature.\r\n";
	}
